...
```{r initialize, warning = TRUE, message = TRUE}
# AirSensor2 package
library(AirSensor2)
...
A quick review of `sort(okanogan_pas$locationName)` reveals:
...
Along with **where** sensors are located, it's important to know **when** they
collected data. (Low-cost sensors don't last forever.) The `pas_lifespanPlot()`
helps with this as seen in the following two examples:

```{r mvcaa_lifespan, warning = FALSE, message = FALSE, fig.height = 10}
...
sensor at "Little Start School" as `"95189"`. (Zoom in on Winthrop to see it.)
From the lifespan plot, we can see that this monitor was first deployed at the
end of 2020 and is still producing data.

The following chunk of code creates a raw _pat_ object for this sensor:
...
and that `pm2.5_atm` values varied from ~20 to ~120.

```{r explore_pat_raw, warning = FALSE, message = FALSE, fig.height = 7}
...
_While useful for engineering-level analysis, it is important to note that high 
resolution, raw data can consume a lot of points interacting with the PurpleAir
API. Creation of hourly data is much more frugal, utilizing the API's ability
to aggregate data._
...
The  `pat_createHourly()` function
only downloads those parameters typically used in QC and correction functions.
...
Below, we create a _pat_hourly_ object for a period in August, 2021 when wildfire 
smoke severely impacted the Methow Valley.

```{r pat_createHourly, warning = FALSE, message = FALSE}
...
The `pat_toMonitor()` function returns a _monitor_ object ready to use with the
...
```{r pat_toMonitor, warning = FALSE, message = FALSE}
trailhead_monitor <-
  trailhead_hourly %>%
  pat_toMonitor()
